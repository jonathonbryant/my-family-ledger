...
function App() {
  ...
  function addTasksBulk(sectionId, titles, defaults) {
    setState(s => ({
      ...s,
      sections: s.sections.map(sec => sec.id === sectionId ? {
        ...sec,
        tasks: [...sec.tasks, ...titles.map(t => ({
          id: uid(),
          title: t.title,
          credits: t.credits != null ? t.credits : defaults.credits,
          freq: defaults.freq,
          deadline: null
        }))]
      } : sec)
  ...
  function resetTaskStatus(taskId) {
    setState(s => {
      ...
      delete completedOnce[taskId];
      const completedPeriod = {
        ...s.completedPeriod
      };
      delete completedPeriod[taskId];
      const lastEarn = s.ledger.find(e => e.kind === "earn" && e.taskId === taskId);
      let balance = s.balance;
      let ledger = s.ledger;
      if (lastEarn) {
        balance = balance - lastEarn.amount;
        ledger = [{
          id: uid(),
          kind: "spend",
          desc: `Reset: ${lastEarn.desc}`,
          amount: -lastEarn.amount,
          ts: Date.now(),
...
function LedgerView({
  ...
  const filtered = state.ledger.filter(e => e.ts >= cutoffs[filter]);
  const filters = [{
    id: "week",
    label: "This week"
  }, {
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    style: styles.emptyState
  }, "No activity in this period."), filtered.map(e => /*#__PURE__*/React.createElement("div", {
    key: e.id,
    style: styles.row
  }, /*#__PURE__*/React.createElement("div", {
    style: {
...
function ManageView({
  ...
  const [bulkOpen, setBulkOpen] = useState({}); // sectionId -> bool
  const [bulkDrafts, setBulkDrafts] = useState({}); // sectionId -> { text, credits, freq }
  ...
  function bulkFor(sectionId) {
    return bulkDrafts[sectionId] || {
      text: "",
      credits: "10",
      freq: "once"
    };
  }
  function setBulk(sectionId, patch) {
    setBulkDrafts(d => ({
      ...d,
      [sectionId]: {
        ...bulkFor(sectionId),
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    style: styles.rowMeta
  }, t.credits, " credits · ", t.freq, t.deadline ? ` · due ${new Date(t.deadline).toLocaleDateString()}` : "")), (state.completedOnce[t.id] || state.completedPeriod[t.id]) && /*#__PURE__*/React.createElement("button", {
    style: styles.linkBtnNeutral,
    onClick: () => resetTaskStatus(t.id)
  ...
  }, "Add task")), !bulkOpen[sec.id] ? /*#__PURE__*/React.createElement("button", {
    style: {
      ...styles.linkBtnNeutral,
      marginTop: 2
    },
    onClick: () => setBulkOpen(o => ({
      ...o,
      [sec.id]: true
    }))
  }, "Bulk add (paste a list)") : /*#__PURE__*/React.createElement("div", {
    style: {
      marginTop: 8,
      padding: "10px 2px",
      borderTop: `1px dashed ${COLORS.paperLine}`
    }
  }, /*#__PURE__*/React.createElement("div", {
    style: {
      ...styles.rowMeta,
  ...
  }, "One assignment per line. Optionally add credits after a comma, e.g. \"Fractions worksheet, 12\" — otherwise the default below is used."), /*#__PURE__*/React.createElement("textarea", {
    style: {
      ...styles.input,
      width: "100%",
      minHeight: 90,
    ...
    placeholder: "Lesson 1: Adding fractions\nLesson 2: Comparing fractions, 15\nReading log entry",
    value: bulkFor(sec.id).text,
    onChange: e => setBulk(sec.id, {
      text: e.target.value
    })
  }), /*#__PURE__*/React.createElement("div", {
    style: styles.draftRow
  ...
  }, "Default credits", /*#__PURE__*/React.createElement("input", {
    style: {
      ...styles.input,
      width: 60
    },
    type: "number",
    value: bulkFor(sec.id).credits,
    onChange: e => setBulk(sec.id, {
      credits: e.target.value
    })
  })), /*#__PURE__*/React.createElement("select", {
    style: styles.input,
    value: bulkFor(sec.id).freq,
    onChange: e => setBulk(sec.id, {
      freq: e.target.value
    })
  }, /*#__PURE__*/React.createElement("option", {
    ...
    style: styles.smallBtnGold,
    onClick: () => {
      const b = bulkFor(sec.id);
      const items = parseBulkLines(b.text);
      if (!items.length) return;
      addTasksBulk(sec.id, items, {
        credits: parseInt(b.credits, 10) || 10,
        freq: b.freq
      });
      setBulkDrafts(d => ({
        ...d,
  ...
  }, "Add all"), /*#__PURE__*/React.createElement("button", {
    style: styles.smallBtnGhost,
    onClick: () => setBulkOpen(o => ({
      ...o,
      [sec.id]: false
    ...
    style: styles.rowMeta
  }, "Clears the entire transaction history (Statement / History tab). The current balance, tasks, and rewards are not affected."), !confirmingClear ? /*#__PURE__*/React.createElement("button", {
    style: {
